...
//FIXME:调试用输出，可删除
//   printk("DEBUG---inode pos---%x",inode_found);
...
/* // 1 回收inode占用的所有块 
   ...
   if (inode_to_del->i_sectors[12] != 0) {
      ...
      group_recycle(part,part->block_group,inode_to_del->i_sectors[12]);
   }
   ...
   while (block_idx < block_cnt) {
      ...
      group_recycle(part,part->block_group,all_blocks[block_idx]);
...
//TODO:总共的块数已经被存储
   uint32_t tmp_lba1=inode_to_del->first_sector_lba,tmp_lba2;//tmp_lba1初始是第一个文件区的地址
   struct sector_file tmp_buf;
   //下面开始把块进行回收
   while(tmp_lba1)
   {
      ide_read(part->my_disk, tmp_lba1,&tmp_buf , 1);
      tmp_lba2=tmp_buf.next_lba;
      group_recycle(part,part->block_group,tmp_lba1);
      tmp_lba1=tmp_lba2;
   }
...
*/
//TODO:只用设置初始扇区地址为0，扇区数为0
   new_inode->first_sector_lba=0;
   new_inode->total_sector_num=0;
