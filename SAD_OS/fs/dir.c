...
*/
//TODO:扇区全部写入all_blocks
   uint32_t block_idx = 0;
   uint32_t tmp_lba=pdir->inode->first_sector_lba;
   struct sector_file* tmp_sec_file=(struct sector_file*)buf;
   while(tmp_lba)
   {
      all_blocks[block_idx++]=tmp_lba;
      ide_read(part->my_disk, tmp_lba,tmp_sec_file , 1);
      tmp_lba=tmp_sec_file->next_lba;
   }
   block_cnt =block_idx;//记录总共的块数
   block_idx=0;//让这个下标回到0
...
//TODO:修改指针指向
   ...
   struct dir_entry* p_de = (struct dir_entry*)(&tmp_sec_file->pad);	    // p_de为指向目录项的指针,值为buf起始地址


   uint32_t dir_entry_size = part->sb->dir_entry_size;

//TODO:修改扇区数据区大小，实际上除法运算后值相同
   //uint32_t dir_entry_cnt = SECTOR_SIZE / dir_entry_size;   // 1扇区内可容纳的目录项个数
   uint32_t dir_entry_cnt = SECTOR_FILE_SIZE / dir_entry_size;   // 1扇区内可容纳的目录项个数
...
//TODO:可以读到buf，但是更好的是读到tmp_sec_file
      //ide_read(part->my_disk, all_blocks[block_idx], buf, 1);
      ide_read(part->my_disk, all_blocks[block_idx], tmp_sec_file, 1);
...
//TODO:修改512为508
   ...
   uint32_t dir_entrys_per_sec = (SECTOR_FILE_SIZE / dir_entry_size);       // 每扇区最大的目录项数目
...
//TODO:进行修改
   //从第一个块开始向后读入
   uint32_t tmp_lba=dir_inode->first_sector_lba,tmp_lba_cpy=0;//tmp_lba_cpy用来保存上一个tmp_lba
   struct sector_file* tmp_sec_file=(struct sector_file*)io_buf;
   struct dir_entry* dir_e = (struct dir_entry*)(&tmp_sec_file->pad);	       // dir_e用来在io_buf中遍历目录项
   //这些扇区都写得有东西，进行判断是否存在空位
   while(tmp_lba)
   {

      //该块已经存在将其读进内存,然后在该块中查找空目录项
      ide_read(cur_part->my_disk, tmp_lba, tmp_sec_file, 1);
      ...
      while (dir_entry_idx < dir_entrys_per_sec) 
      {
         ...
         {	
	         ...
	         ide_write(cur_part->my_disk, tmp_lba, tmp_sec_file, 1);
      ...
      //找不到空目录项，换到下一个扇区
      tmp_lba_cpy=tmp_lba;//存储上一个扇区号
      tmp_lba=tmp_sec_file->next_lba;//获取下一个扇区号

   }

   //上面的循环执行完后，此时的tmp_lba=0，tmp_lba_cpy为最后一个扇区的扇区号
   //此时的tmp_sec_file为最后一个扇区的内容

   //前面的扇区如果都没有空位了，就分配新的扇区
   ...
   //分配后记录新的扇区号，写入现在最后一个扇区的next_lba
   tmp_sec_file->next_lba=block_lba;
   //将原本的最后一个扇区写入内存
   ide_write(cur_part->my_disk, tmp_lba_cpy, tmp_sec_file, 1);
   // 再将新目录项p_de写入新分配的间接块
	 memset(tmp_sec_file, 0, 512);
    tmp_sec_file->next_lba=0;//下一个扇区号设为0
	 memcpy(tmp_sec_file, p_de, dir_entry_size);//写入目录信息
	 ide_write(cur_part->my_disk,block_lba , io_buf, 1);//把新加入的扇区也写入内存
    //增加父亲i结点中的i_size和总扇区数
	 dir_inode->i_size += dir_entry_size;
    dir_inode->total_sector_num+=1;
	 return true;
...
/*
   ...
   while (block_idx < 140) {  // 文件(包括目录)最大支持12个直接块+128个间接块＝140个块
	 ...
	 if (block_idx < 12) {	    // 若是直接块
	    ...
	    if (block_lba == -1) {
         group_recycle(cur_part,cur_part->block_group,dir_inode->i_sectors[12]);
...
*/
//TODO:修改
   uint32_t tmp_lba=dir_inode->first_sector_lba;
   struct sector_file* tmp_sec_file=(struct sector_file*)io_buf;
   //现在开始全部写入
   while(tmp_lba)
   {
...
//TODO:
      ...
      memset(tmp_sec_file, 0, SECTOR_SIZE);
      // 读取扇区,获得目录项
      ide_read(part->my_disk, all_blocks[block_idx], tmp_sec_file, 1);
   ...
   // 在此扇区中找到目录项后,清除该目录项并判断是否回收扇区,随后退出循环直接返回
    ...
    // 除目录第1个扇区外,若该扇区上只有该目录项自己,则将整个扇区回收
	 ...
	 // a 在块位图中回收该块
      group_recycle(cur_part,cur_part->block_group,all_blocks[block_idx]);
...
*/
//TODO:在上一个扇区中把块地址替换为下一个块
      ide_read(part->my_disk, all_blocks[block_idx-1], tmp_sec_file, 1);
      tmp_sec_file->next_lba=all_blocks[block_idx+1];
      ide_write(part->my_disk, all_blocks[block_idx-1], tmp_sec_file, 1);
      //还要把sector数量减一
      dir_inode->total_sector_num-=1;
	 ...
	 ide_write(part->my_disk, all_blocks[block_idx], tmp_sec_file, 1);
      }
...
//TODO:把dir_buf拿来用用
   ...
   struct sector_file* tmp_sec_file=(struct sector_file*)(dir->dir_buf);
...
//TODO:更改
   ...
   uint32_t dir_entrys_per_sec = SECTOR_FILE_SIZE / dir_entry_size;	 // 1扇区内可容纳的目录项个数
...
*/
//TODO:只需要判断总共扇区数即可
   ASSERT(child_dir_inode->total_sector_num==1);
   ...
   /* 回收inode中i_secotrs中所占用的扇区,并同步inode_bitmap和block_group */
