...
int32_t block_bitmap_alloc(struct partition* part) {
   ...
   return group_allocate(part,part->block_group);


   /*
   int32_t bit_idx = bitmap_scan(&part->block_group, 1);
   if (bit_idx == -1) {
      return -1;
   }
   bitmap_set(&part->block_group, bit_idx, 1);
...
/* 需要被同步到硬盘的位图只有inode_bitmap和block_group */
   switch (btmp_type) {
      case INODE_BITMAP:
   ...
   /*
      case BLOCK_BITMAP: 
	 sec_lba = part->sb->block_group_lba + off_sec;
	 bitmap_off = part->block_group.bits + off_size;
	 break;
   */
   }
...
//FIXME:调试用输出，可删除
//   printk("DEBUG---inode pos---%x",new_file_inode);
//  printk("running_thread()->pgdir:%x",running_thread()->pgdir);
...
/*
   ...
   uint32_t block_bitmap_idx = 0;   // 用来记录block对应于block_group中的索引,做为参数传给bitmap_sync
...
//TODO:修改未验证
 ...
 if ((file->fd_inode->i_size + count) > (BLOCK_SIZE * 140))	{   // 文件目前最大只支持512*140=71680字节
   ...
   //在这里修改为sector_file*
   struct sector_file* io_buf = sys_malloc(BLOCK_SIZE);
   if (io_buf == NULL) {
      printk("file_write: sys_malloc for io_buf failed\n");
      return -1;
   }
   //仍然沿用140个块的最大值
   uint32_t* all_blocks = (uint32_t*)sys_malloc(BLOCK_SIZE + 48);	  // 用来记录文件所有的块地址,仍然假定为140个位子
   if (all_blocks == NULL) {
      printk("file_write: sys_malloc for all_blocks failed\n");
      return -1;
   }
   ...
   // 判断文件是否是第一次写,如果是,先为其分配一个块
   if (file->fd_inode->first_sector_lba== 0) {
      block_lba = block_bitmap_alloc(cur_part);
      if (block_lba == -1) {
	      printk("file_write: block_bitmap_alloc failed\n");
	      return -1;
      }
      file->fd_inode->first_sector_lba = block_lba;
      file->fd_inode->total_sector_num=1;
      //并且还要把这个块的next_block置为0
      memset(io_buf,0,BLOCK_SIZE);
   ...
   // 写入count个字节前,该文件已经占用的块数
   uint32_t file_has_used_blocks = file->fd_inode->i_size / SECTOR_FILE_SIZE + 1;

   // 存储count字节后该文件将占用的块数
   uint32_t file_will_use_blocks = (file->fd_inode->i_size + count) / SECTOR_FILE_SIZE + 1;
   ASSERT(file_will_use_blocks <= 140);
   ...
   //先把先前已经存在的块全部写入
   uint32_t tmp_lba=file->fd_inode->first_sector_lba;
   uint32_t i=0;
   while(tmp_lba)
   {
      ide_read(cur_part->my_disk, tmp_lba, io_buf, 1);
      all_blocks[i++]=tmp_lba;
      tmp_lba=io_buf->next_lba;
   }

   //此时的io_buf为最后一个扇区的内容，下一个扇区号应该为0

   //再添加需要分配的扇区号进入all_blocks
   //共分配add_blocks个扇区
   for(uint32_t j=0;j<add_blocks;j++)
   {
      //分配一个扇区
      block_lba = block_bitmap_alloc(cur_part);
      if (block_lba == -1) 
...
//TODO:增加总共扇区的数量
      file->fd_inode->total_sector_num++;

      //之前连接的最后一个扇区的下一个地址变成刚刚分配的扇区
      //如果j==0,就不需要memset处理，因为io_buf现在存储的内容是是之前已经存在的最后一个扇区
      //因此当j==0的时候，恰好可以让之前已经存在的块的next_block指向新分配的第一个块
      if(j!=0)
      {
         memset(io_buf,0,BLOCK_SIZE);
      }
      io_buf->next_lba=block_lba;
      //将这个扇区写入磁盘all_blocks[i-1]
      ide_write(cur_part->my_disk, all_blocks[i-1], io_buf, 1);
      //将这个扇区号写入all_blocks[i]
      all_blocks[i++]=block_lba;
   }
   //最后，all_blocks[i-1]还没有写入磁盘。all_blocks[i]的值应该为0
   memset(io_buf,0,BLOCK_SIZE);
   ide_write(cur_part->my_disk, all_blocks[i-1], io_buf, 1);
...
// 用到的块地址已经收集到all_blocks中,下面开始写数据
   ...
   while (bytes_written < count) {      // 直到写完所有数据
      memset(io_buf, 0, BLOCK_SIZE);
      sec_idx = file->fd_inode->i_size / SECTOR_FILE_SIZE;

      ASSERT(sec_idx<=i-1);//之前all_blocks只写入到了下标为i-1的位置，因此最多访问到第i-1个位置

      sec_lba = all_blocks[sec_idx];
      sec_off_bytes = file->fd_inode->i_size % SECTOR_FILE_SIZE;
      sec_left_bytes = SECTOR_FILE_SIZE - sec_off_bytes;
      ...
      memcpy(io_buf->pad + sec_off_bytes, src, chunk_size);
      ide_write(cur_part->my_disk, sec_lba, io_buf, 1);
...
//TODO:修改未验证
   ...
   uint32_t block_read_start_idx = file->fd_pos / SECTOR_FILE_SIZE;		       // 数据所在块的起始地址
   uint32_t block_read_end_idx = (file->fd_pos + size) / SECTOR_FILE_SIZE;	       // 数据所在块的终止地址
   uint32_t read_blocks = block_read_start_idx - block_read_end_idx;	       // 如增量为0,表示数据在同一扇区
   ASSERT(block_read_start_idx < 139 && block_read_end_idx < 139);

   uint32_t block_idx;		       // 获取待读的块地址

   //下面开始读取数据区的扇区号并放到all_block里面，从首个区块号开始
   //一直读取直到到达数据所在块的中止地址
   uint32_t tmp_lba=file->fd_inode->first_sector_lba;
   uint32_t i=0;//当前all_block的下标
   while(tmp_lba!=block_read_end_idx+1 && tmp_lba!=0)
   {
      ide_read(cur_part->my_disk, tmp_lba, io_buf, 1);
      all_blocks[i++]=tmp_lba;
      tmp_lba=io_buf->next_lba;
   ...
   uint32_t bytes_read = 0;
   while (bytes_read < size) {	      // 直到读完为止
      sec_idx = file->fd_pos / SECTOR_FILE_SIZE;

      ASSERT(sec_idx<=i-1);//之前all_blocks只写入到了下标为i-1的位置，因此最多访问到第i-1个位置

      sec_lba = all_blocks[sec_idx];
      sec_off_bytes = file->fd_pos % SECTOR_FILE_SIZE;
      sec_left_bytes = SECTOR_FILE_SIZE - sec_off_bytes;
      chunk_size = size_left < sec_left_bytes ? size_left : sec_left_bytes;	     // 待读入的数据大小

      memset(io_buf, 0, BLOCK_SIZE);
      ide_read(cur_part->my_disk, sec_lba, io_buf, 1);
      memcpy(buf_dst, io_buf->pad + sec_off_bytes, chunk_size);
