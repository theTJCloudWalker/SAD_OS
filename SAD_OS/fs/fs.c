...
static bool mount_partition(struct list_elem* pelem, int arg) {
   ...
   if (!strcmp(part->name, part_name)) {
	 ...
	 PANIC("alloc memory failed!");
      ...
      /**********     将硬盘上的块位图读入到内存    ****************/
      cur_part->block_group = (uint8_t*)sys_malloc(sb_buf->block_group_sects * SECTOR_SIZE);
      if (cur_part->block_group == NULL) {
	 PANIC("alloc memory failed!");
      }
      //cur_part->block_group.btmp_bytes_len = sb_buf->block_group_sects * SECTOR_SIZE;
      /* 从硬盘上读入块位图到分区的block_group.bits */
      ide_read(hd, sb_buf->block_group_lba, cur_part->block_group, sb_buf->block_group_sects);   
      /*************************************************************/
...
/* 为方便实现,一个块大小是一扇区 */
   ...
   //uint32_t inode_table_sects = DIV_ROUND_UP(((sizeof(struct inode) * MAX_FILES_PER_PART)), SECTOR_FILE_SIZE);
...
/*********************************************************/
   ...
   sb.block_group_lba = sb.part_lba_base + 2;	 // 第0块是引导块,第1块是超级块
   sb.block_group_sects = block_group_sects;

   sb.inode_bitmap_lba = sb.block_group_lba + sb.block_group_sects;
   ...
   printk("%s info:\n", part->name);
   printk("   magic:0x%x\n   part_lba_base:0x%x\n   all_sectors:0x%x\n   inode_cnt:0x%x\n   block_group_lba:0x%x\n   block_bitmap_sectors:0x%x\n   inode_bitmap_lba:0x%x\n   inode_bitmap_sectors:0x%x\n   inode_table_lba:0x%x\n   inode_table_sectors:0x%x\n   data_start_lba:0x%x\n", sb.magic, sb.part_lba_base, sb.sec_cnt, sb.inode_cnt, sb.block_group_lba, sb.block_group_sects, sb.inode_bitmap_lba, sb.inode_bitmap_sects, sb.inode_table_lba, sb.inode_table_sects, sb.data_start_lba);
...
/* 找出数据量最大的元信息,用其尺寸做存储缓冲区*/
   uint32_t buf_size = (sb.block_group_sects >= sb.inode_bitmap_sects ? sb.block_group_sects : sb.inode_bitmap_sects);
   buf_size = (buf_size >= sb.inode_table_sects ? buf_size : sb.inode_table_sects) * SECTOR_SIZE;
   uint8_t* buf = (uint8_t*)sys_malloc(buf_size);	// 申请的内存由内存管理系统清0后返回
   
/**************************************
 * 2 将块位图初始化并写入sb.block_group_lba *
 *************************************/
...
/*
   ...
   */
//已进行，进行块位图初始化
//分一个块给根目录,跳过data_start_lba的地址
   group_disk_init(part,part->block_group,sb.block_group_lba,sb.data_start_lba+1,free_sects - block_group_sects-1);

   //ide_write(hd, sb.block_group_lba, buf, sb.block_group_sects);

   printk("位图初始化data_start_lba:0x%x,扇区大小为%d\n", sb.data_start_lba + 1,free_sects - block_group_sects-1);
 ...
 ***************************************/
    ...
    * 无须再像block_group那样单独处理最后一扇区的剩余部分,
...
//TODO:这里修改
   ...
   i->first_sector_lba = sb.data_start_lba;	     // 由于上面的memset,i_sectors数组的其它元素都初始化为0 
   i->total_sector_num=1;
...
//TODO:在这里强行修改为指向tmp_file->pad，同时修改下一个文件的盘块号
   //struct dir_entry* p_de = (struct dir_entry*)buf;
   struct sector_file* tmp_file=(struct sector_file*)buf;
   tmp_file->next_lba=0;
   struct dir_entry* p_de = (struct dir_entry*)(&(tmp_file->pad));
   ...
   /*
   int32_t root_dir_lba=block_bitmap_alloc(part);
   if(root_dir_lba==-1){
      printk("初始化分配根目录出错");
      while(1);
   }
   ide_write(hd, root_dir_lba, buf, 1);
   */

//TODO:这里进行修改写入磁盘的格式
//ide_write(hd, sb.data_start_lba, buf, 1);
   ide_write(hd, sb.data_start_lba, tmp_file, 1);
...
int32_t sys_mkdir(const char* pathname) {
   ...
   uint32_t block_bitmap_idx = 0;     // 用来记录block对应于block_group中的索引
   int32_t block_lba = -1;
/* 为目录分配一个块,用来写入目录.和.. */
...
//TODO:这里修改
   ...
   new_dir_inode.first_sector_lba = block_lba;
   new_dir_inode.total_sector_num=1;
...
//TODO:这里修改
   ...
   struct sector_file* tmp_sec_file=(struct sector_file*)io_buf;
   struct dir_entry* p_de = (struct dir_entry*)(&tmp_sec_file->pad);
...
//TODO:这里修改
   ...
   tmp_sec_file->next_lba=0;
   ide_write(cur_part->my_disk, new_dir_inode.first_sector_lba, tmp_sec_file, 1);
   ...
   memset(io_buf, 0, SECTOR_SIZE * 2);	 // 清空io_buf
   if (!sync_dir_entry(parent_dir, &new_dir_entry, io_buf)) {	  // sync_dir_entry中将block_group通过bitmap_sync同步到硬盘
...
//TODO:进行修改
   ...
   uint32_t block_lba = child_dir_inode->first_sector_lba;
...
//TODO:进行修改
   ...
   struct sector_file* tmp_sec_file=(struct sector_file*)io_buf;
   struct dir_entry* dir_e = (struct dir_entry*)(&tmp_sec_file->pad);
   /* 第0个目录项是".",第1个目录项是".." */
...
//TODO:全部写入
   ...
   uint32_t tmp_lba=parent_dir_inode->first_sector_lba;
   struct sector_file* tmp_sec_file=(struct sector_file*)io_buf;//借用io_buf的地址
   while(tmp_lba)
   {
      all_blocks[block_cnt++]=tmp_lba;//每读入一个块号，block_cnt++

      ide_read(cur_part->my_disk,tmp_lba , tmp_sec_file, 1);//读取数据获得下一个块号
      tmp_lba=tmp_sec_file->next_lba;
   }
...
//TODO:数据区没得512了。而是508，但是好像算出来都是一个值
   //uint32_t dir_entrys_per_sec = (512 / dir_entry_size);
   uint32_t dir_entrys_per_sec = (SECTOR_FILE_SIZE / dir_entry_size);
...
//TODO:不读到io_buf，读到tmp_sec_file，虽然地址一样
	 //ide_read(cur_part->my_disk, all_blocks[block_idx], io_buf, 1);
    ide_read(cur_part->my_disk, all_blocks[block_idx], tmp_sec_file, 1);
...
void sys_help(void) {
 ...
 buildin commands:\n\
       ...
       clear: clear screen\n\
       pikachu:show a pikachu on the screen\n\
 shortcut key:\n\
       ctrl+l: clear screen\n\
       ctrl+u: clear input\n\n");
}
...
void filesys_init() {
   ...
   printk("channel_no:%d,channel_cnt:%d",channel_no,channel_cnt);
   ...
   printk("now to mount part");

   /* 挂载分区 */
   list_traversal(&partition_list, mount_partition, (int)default_part);

   printk("now to open the root dir");
...
//FIXME:不知道是不是可以用
//专门用来创建文件
void sys_touch(const char* pathname)
{
